...
- **`acad-underlays.attach_pdf_underlay` built, second pass, 24/24 live, bank 686 → 687.** The
  first underlays tranche withheld this tool for lack of a PDF to verify against, not an API
  problem. A zero-cost compiler probe (`_ApiProbe.cs` / `ACADMCP_API_PROBE`, no AutoCAD needed)
  confirmed `PdfReference`/`PdfDefinition` mirror `DgnReference`/`DwfReference` exactly, so this
  reuses the same `AttachUnderlay<TDef,TRef>` generic path already proven by
  `attach_dwf_underlay` - one more branch on `DictKeyFor`/`KindOf`, and `list_underlays` now scans
  a third definition dictionary. `list_underlays`/`detach_underlay`/`clip_underlay`/
  `set_underlay_adjust` needed no changes at all: the shared `UnderlayReference` base already
  covered the new type.

  **The fixture is generated by this bank's own already-verified `files.export_file`**
  (`scripts/fixtures/pdf-underlay-test/`, source and a regeneration recipe committed), never a
  personal document off the machine, even though real PDFs exist here - a 300x100 rectangle with
  a circle near one corner, deliberately non-square (rule 26 §14).

  **No AutoCAD API defect found this tranche - the one failure on first run was the verification
  script's own wrong assumption.** It expected the underlay's aspect ratio to match the source
  drawing's 3:1. Measured: AutoCAD's PDF exporter fits the drawing onto a default page (US
  Letter, `612 792`) rather than cropping to content, so the real aspect is the page's 0.773.
  Still non-square, still able to catch a width/height swap - the check now reads the PDF's own
  `/MediaBox` straight out of the file bytes at runtime as ground truth, rather than assuming a
  ratio a second time.
