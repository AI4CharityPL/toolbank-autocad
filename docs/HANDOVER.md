...
**687 built.** Roughly 18 identified as remaining, and nearly all of it is gated rather than hard —
this session cleared everything that wasn't.

**2026-08-11: `acad-underlays.attach_pdf_underlay` built, second pass, 24/24 live, 6 tools now,
one restart.** The first underlays pass withheld this for lack of a PDF to verify against, not an
API problem. A zero-cost compiler probe confirmed `PdfReference`/`PdfDefinition` mirror
`DgnReference`/`DwfReference` exactly, so this reuses the same `AttachUnderlay<TDef,TRef>` generic
path with one more branch on `DictKeyFor`/`KindOf`. **The fixture is generated by this bank's own
already-verified `files.export_file`** (`scripts/fixtures/pdf-underlay-test/`), deliberately never
a personal document off the machine, even though real PDFs exist here. One thing measured wrong on
the first verification run, not a code defect: the exporter fits a drawing onto a default page
(US Letter, `612 792`) rather than cropping to content, so the underlay's real aspect is the
page's 0.773, not the source drawing's 3:1 — the check now reads the PDF's own `/MediaBox` at
runtime instead of assuming a ratio. No AutoCAD API defect found this tranche.
