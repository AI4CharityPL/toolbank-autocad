...
| `netload_assembly` | lisp | **Not attempted in the 2026-08-11 tranche.** Needs the identical command-context fix as `run_command_sequence`, but writing it was blocked by this session's own safety classifier — dynamically loading an arbitrary DLL into the running AutoCAD process reads as a code-loading primitive distinct from queuing drawing commands, and the classifier refused the file write outright. `run_command_sequence` and `run_script_file` (see above; `run_script_file` was subsequently withdrawn on unrelated grounds) went through once `netload_assembly`'s `DynamicLinker.LoadModule` call was dropped from the same file. Revisit only with the user's explicit go-ahead for that specific capability. |
