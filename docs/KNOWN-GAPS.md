...
| `set_image_transparency` | images | **No managed API at all.** The background-transparency toggle for bitonal raster images (AutoCAD's own "Transparency" property in the image Properties palette) has no exposed member under any name — nine were tried against the compiler: `RasterImage.IsTransparent`, `.ImageTransparency`, `.IsBackgroundTransparent`, `.Transparent`, `.ShowTransparent`, `RasterImageDef.IsTransparent`, `.IsBitonal`, `.ImageIsTransparent`, `.DisplayOpaqueBackground`. All nine fail `CS1061`, while `Brightness`/`Contrast`/`Fade` on the same type compile clean in the same probe — so this is the property being absent, not the object. Same shape as `set_mtext_frame` and arc-aligned text. |
| `reorder_image_draworder` | images | **Struck as a duplicate, not deferred.** `modify.set_draworder` already reorders any entity by handle, images included — a second name over the same operation would give a router two ways to spell one action. |
| `set_render_environment` (fog) | render/lights | **Struck 2026-08-11 after three probe rounds.** `RenderEnvironment` is a real `DBObject` with `FogEnabled`/`FogColor`, but no persistence route resolves: `Database.RenderEnvironment` does not exist, and neither does `RenderEnvironmentId` on `ViewportTableRecord` or `ViewTableRecord` — the pattern that worked for `Sun` does not repeat here. More to the point, even a working version would set a value nothing in this bank can ever be shown to affect: fog is resolved by a RENDERER, and `files.export_file`'s only image route is a shaded-viewport capture (`View.GetSnapShot`) that does not render fog, materials or shadows photorealistically. Same "settings for a renderer that cannot be invoked" reasoning as `set_render_preset`/`set_render_quality`/`set_exposure`/`list_render_history`. |
| `surface_curvature_analysis`, `surface_draft_analysis` | surfaces | **Resolved and struck 2026-08-11 - left as an open `?` since this phase was first written.** `ANALYSISCURVATURE`/`ANALYSISDRAFT` are real AutoCAD commands, so this was a genuine open question rather than a guess. Bulk-probed across every plausible spelling and namespace (rule 26 §12c): `Entity.AnalysisMesh`, `Entity.AnalysisStyle`, and an `AnalysisStyle` type under `DatabaseServices`, `GraphicsInterface`, `Colors` and `ApplicationServices` all fail to compile. There is no DB-persisted, readable-back analysis object anywhere in the managed API - these are viewport display modes with nothing an API caller can observe, the same shape as `set_render_environment` above. |
| `run_script_file` | lisp | **Withdrawn 2026-08-11 after two measured fix attempts.** Built on the same `LispCommandBridge` as `run_command_sequence` (rule 26 §15/§22) — a queued `[CommandMethod]` giving `Editor.Command` a genuine COMMAND context. Even a `.scr` containing one plain `CIRCLE` drew nothing. First suspect, `_.SCRIPT` opening its file-picker dialog unless `FILEDIA=0`, was tried and ruled out: a captured before/after baseline confirmed the fix correctly forced 0 and restored the original value, and the script still did nothing. Likely the same "second round of input silently drops" limit that also affects a chained second command in `run_command_sequence` (rule 26 §22), this time inside `_.SCRIPT`'s own internal line replay — a hypothesis, not a measurement. Not shipped on an unconfirmed theory. `run_command_sequence` remains the way to reach this bank's escape hatch for a single command. |
| `attach_dgn_underlay` | underlays | **Withheld 2026-08-11, blocked on a file, not the API.** Every `.dgn` on this machine (10 files, all `UserDataCache\Template\*-Seed*.dgn`) is an export seed template with no real content, and every one refuses to load with `eInvalidInput` regardless of `itemName`. `attach_dwf_underlay` uses the identical generic `AttachUnderlay<TDef,TRef>` code path and works cleanly against a real Sheet Set DWF, which is strong evidence the DGN side is correct and only the available files are unsuitable. Unblock with one real (non-seed) `.dgn`. |
| `list_underlay_layers`, `set_underlay_layer_visibility` | underlays | **Struck, not deferred — 7 candidate names tried.** No per-layer visibility control compiles against `UnderlayReference` or `UnderlayDefinition` under any of `GetLayers`, `SetLayer`, `IsLayerVisible`, `UnderlayLayers`, `SubItems`, `LayerNames` (both types). |
| `bind_underlay` | underlays | **Struck.** `UnderlayReference.Bind()` does not exist; no candidate route was found. |
| `acad-pointclouds` — all 12 tools (`attach_point_cloud`, `list_point_clouds`, `detach_point_cloud`, `clip_point_cloud`, `invert_pointcloud_clip`, `set_pointcloud_density`, `set_pointcloud_colormap`, `set_pointcloud_stylization`, `extract_pointcloud_section`, `pointcloud_to_geometry`, `set_pointcloud_crop_state`, `get_pointcloud_info`) | pointclouds | **Whole category not built — blocked on a scan file, not the API.** Every one of these hangs off `attach_point_cloud`, so there is nothing to list, clip, stylize or measure until a cloud is attached, and attaching needs a real `.rcp` or `.rcs`. None exists anywhere on this machine and ReCap itself is not installed, so one cannot even be produced locally — the formats are Autodesk's own. Two reconnaissance rounds confirm the API is **not** the obstacle: `PointCloudEx`, `PointCloud`, `PointCloudDefEx`, `PointCloudCrop`, `PointCloudCropType`, `PointCloudStylizationType` and `PointCloudColorMap` all resolve, along with most of their properties — only the attach entry point and the crop add/read methods haven't been named yet, which reads as a third probe round away, not a wall. Full reconnaissance notes in [COVERAGE-ROADMAP.md §4.5](COVERAGE-ROADMAP.md). **Unblock with:** one `.rcp`/`.rcs` file anywhere on disk — Autodesk publishes free ReCap sample scans, and a single-scan `.rcs` is enough, since the checks these tools need (point counts, extents before/after a crop, the count dropping when a clip is inverted) need one cloud, not a good one. Good first contribution for someone with ReCap or a sample scan handy; candidate for a follow-up release once unblocked. |
| `set_material_map`, `set_material_mapping`, `load_material_library` | materials | **3 of the category's 8 planned tools withheld — blocked on files, not the API.** `set_material_map`/`set_material_mapping` need a real texture image to apply and verify against, and `load_material_library` needs a real `.adsklib`; none of either exists anywhere on this machine (checked 2026-08-11 alongside the DGN/DWF/IES search). The other six materials tools shipped and are live (`create_material`, `modify_material`, `list_materials`, `assign_material`, `unassign_material`, `delete_material` — 37/37 verified). **Unblock with:** any texture image (`.png`/`.jpg`) for the map tools, and one `.adsklib` (Autodesk ships sample material libraries) for `load_material_library`. Good first contribution; candidate for a follow-up release once unblocked. |
