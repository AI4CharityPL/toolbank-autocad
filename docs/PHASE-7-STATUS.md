...
Single source of truth for the team and Cursor agents on what shipped, how it was verified, and what's explicitly out of scope for v1. Historical detail and tool lists: [CHANGELOG.md](../CHANGELOG.md). Current known gaps: see [README.md § Known Limitations](../README.md#known-limitations).
...
- **Architecture — wall openings** ✅. `insert_door`/`insert_window` never actually cut the host wall (a real bug, `.cursor/rules/36-architecture-domain-traps.mdc` §3 explicitly required it) — both now accept an optional `wallHandle` and cut the wall at their own jambs/axis span via `split_wall_at_opening` before drawing, verified live (real split wall segments with the correct gap length).
...
- **Parametric — geometric + dimensional constraint types** ⚠️ code shipped, application currently broken. Tangent/Concentric/Collinear/Equal/Symmetric geometric constraints and Linear/Aligned dimensional constraints are implemented, but live-testing found that **every** constraint-application tool in this category fails against real AutoCAD with `eInvalidInput` from `Editor.Command` — see [README § Known Limitations](../README.md#known-limitations) for the full detail.
...
2. Follow the **always-apply** rule `54-development-status.mdc` (in `.cursor/rules/`).
