...
| `create_view_category` | Create a view category in a sheet set - the grouping sheet views are filed under, such as Sections or Details. Needed before set_sheet_view_category has anywhere to file a view. Refuses a name already in use. Writes to the shared .DST under a lock. |
...
| `set_sheet_view_category` | File a sheet view under a view category, moving it out of whatever category it was in. Identify the view by its name or its view number, and the category by name; the category must already exist, and the refusal lists the ones that do. Answers with the previous category so the change is reversible from its own result. Writes to the shared .DST under a lock. |
