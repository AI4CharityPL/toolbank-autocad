# MCPBank manifest format

MCPBank manifest format. One file per category. Required fields and validation.

Each category has exactly one manifest file: `mcpbank-manifests/acad-<name>.json`. The router has its own (`acad-router.json`).

These files are merged into `C:/Users/DELL/mcpbank/registry/mcpd-registry.json` by `scripts/register-mcps.ps1`.
