...
public static class ArchitectureTools
{
    ...
    [McpTool("insert_door",
        "THE ARCHITECTURE ONE: draws the door as primitives on this layer standard, and optionally cuts the wall. WARNING: this door is INVISIBLE to generate_door_schedule, audit_all_rooms, get_room_data and list_openings_in_model - they only see numbered BLOCKs. openings.insert_door places that block AND (since 2026-08-12) can also cut the wall via its own wallHandle argument, so if the door might ever need a schedule entry or an audit to count it, use openings.insert_door instead - this one is for quick graphic-only floor plans where no schedule will ever be generated. Insert a door at a hinge point. Draws the door panel (rectangle width × frameThicknessMm on A-DOOR) at the requested opening angle plus a swing arc (default quarter-circle, on A-DOOR-SWING). swingDirection='left' (default) hinges on the LEFT side of the wall axis; 'right' hinges on the RIGHT. Pass wallHandle to also cut the host wall at the door's jambs (hinge -> hinge + widthMm along hingeAngleDeg) before drawing the panel -- omit it to only draw the door primitives without touching any wall (e.g. when the wall was already cut separately via split_wall_at_opening).",
        "architecture",
        Intent = new[]
        {
    ...
    [McpTool("insert_window",
        "THE ARCHITECTURE ONE: draws the window as primitives on this layer standard. WARNING: this window is INVISIBLE to generate_window_schedule, audit_all_rooms, get_room_data and list_openings_in_model - they only see numbered BLOCKs. openings.insert_window places that block AND (since 2026-08-12) can also cut the wall via its own wallHandle argument, so if the window might ever need a schedule entry or an audit to count it, use openings.insert_window instead - this one is for quick graphic-only floor plans where no schedule will ever be generated. Insert a window centred at a point along a wall axis. Draws 5 entities on A-GLAZ: the sill line (wall side closer to exterior), the glass line (in the middle of the wall), the header line (wall side closer to interior), and two perpendicular jamb lines closing the opening. Pass wallHandle to also cut the host wall at the window's own axis span before drawing -- omit it to only draw the window primitives without touching any wall. rotationDeg is the wall's heading in degrees (0 = horizontal, +90 = vertical going up).",
        "architecture",
        Intent = new[]
        {
            "wstaw okno w sciane",
