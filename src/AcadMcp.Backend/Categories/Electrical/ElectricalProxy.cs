// Composition helper for acad-electrical (rule 35 §2). Calls primitives from
// acad.geometry2d.* / acad.layers.* / acad.annotations.* over IPluginGateway.
// Adds DrawHatchAsync (used to fill the wire-junction dot) and DrawCircleAsync.
...
internal static class ElectricalProxy
{
    ...
    public static async Task<EntityHandle> DrawHatchAsync(
        IPluginGateway gw,
        IReadOnlyList<string> boundaryHandles, string pattern, double scale, double angleDeg, string layer,
        CancellationToken ct)
    {
        var args = new JsonObject
        {
