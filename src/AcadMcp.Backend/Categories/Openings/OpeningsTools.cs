...
public static class OpeningsTools
{
    ...
    [McpTool("list_openings_in_model",
        "Enumerate all opening BlockReferences currently in model-space (block names starting with 'DOOR-' or 'WIN-'). kind='doors'|'windows'|'all'. Optional layerFilter. Returns handle, blockName, kind, number, type, width/height, rei, rc, fireClass, acousticDb, leadShielded, roomFrom, roomTo, position, rotation, layer. Read-only.",
        "openings",
        Intent = new[]
        {
    ...
    [McpTool("export_schedule",
        "Export a door or window schedule to CSV or JSON string (optionally write to disk). kind='doors'|'windows'|'all'. format='csv'|'json'. CSV columns: NUMBER,TYPE,WIDTH_MM,HEIGHT_MM,REI,RC,FIRE_CLASS,ACOUSTIC_DB,LEAD,ROOM_FROM,ROOM_TO,LAYER,HANDLE. Returns the rendered content (also written to outputPath when supplied). Read-only.",
        "openings",
        Intent = new[]
        {
