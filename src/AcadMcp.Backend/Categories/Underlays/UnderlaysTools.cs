...
//
// PDF underlays FIXED 2026-08-11 (second pass): PdfReference/PdfDefinition were confirmed, via a
// zero-cost compiler probe, to mirror the DgnReference/DwfReference shape exactly - same
// AttachUnderlay<TDef,TRef> generic path, one more branch on DictKeyFor/KindOf. attach_pdf_underlay
// ships. The blocker the first tranche cited was a fixture, not the API: no PDF was available to
// verify against then. This one uses a PDF generated by this bank's OWN already-verified
// files.export_file, never a personal document off the machine - see
// scripts/fixtures/pdf-underlay-test/README.md.
//
...
public static class UnderlaysTools
{
    ...
    [McpTool("attach_pdf_underlay", "Attach a PDF file as an underlay reference. itemName selects which page inside the PDF to show - a multi-page PDF holds several; omit it to use the file's default. Refuses a missing file. A name already in use is refused UNLESS it points at the exact same file and item, in which case this is a second placement sharing the existing definition (reusedDefinition: true) - same behaviour as attach_dwf_underlay/attach_image.", "underlays",
        Intent = new[] { "attach a pdf underlay", "insert a pdf file as a reference",
                         "dolacz podklad pdf", "attach a pdf reference",
                         "wstaw plik pdf jako podklad", "reference a pdf page",
                         "underlay a pdf file" },
        RequiresPlugin = true)]
    public static Task<UnderlayAttachResult> AttachPdfUnderlay(IPluginGateway gw, UnderlayAttachArgs args, CancellationToken ct)
        => UnderlaysProxy.CallAsync<UnderlayAttachArgs, UnderlayAttachResult>(gw, "acad.underlays.attach_pdf_underlay", args, T_NORMAL, ct);

    [McpTool("list_underlays", "List DGN, DWF and PDF underlay references attached to the drawing: kind (dgn/dwf/pdf), source path, item name, insertion point, rotation, scale, whether clipped, and the contrast/fade/monochrome adjustment. Read-only.", "underlays",
    ...
    [McpTool("detach_underlay", "Remove one underlay reference (DGN, DWF or PDF) from the drawing. If no other reference still uses the same source definition, the definition is removed too and defRemoved is true; if another placement of the same file/item remains, only this entity goes.", "underlays",
