// Smoke + regression test for the acad-geo category.
...
public class GeoTests
{
    ...
    {
        // Phase 5.3. Struck after reconnaissance rather than left unbuilt: insert_map_image and
        // set_map_image_type (there is NO GeoMap type in this API), set_north_direction
        // (NorthDirection is read-only and is an ANGLE, derived from the design and reference
        // points), and list_coordinate_systems / set_coordinate_system, which collapse into
        // set_geographic_location - AutoCAD exposes no catalogue of systems to enumerate.
        "set_geographic_location", "get_geographic_location", "remove_geolocation",
        "convert_wcs_to_geo", "convert_geo_to_wcs",
    ...
    {
        var tools = NewRegistry().ToolsFor("geo").Select(t => t.Name).OrderBy(n => n).ToArray();
    ...
    {
        foreach (var t in NewRegistry().ToolsFor("geo"))
        {
